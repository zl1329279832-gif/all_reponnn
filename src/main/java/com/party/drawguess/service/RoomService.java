...
public class RoomService {
    ...
    @Transactional
    public RoomSnapshotResponse rematch(RematchRequest request) {
        Room room = getById(request.getRoomId());
        Player owner = playerService.getById(request.getOwnerId());

        if (room.getStatus() != RoomStatus.FINISHED) {
            throw new GameException("对局尚未结束，无法再来一局");
        }

        if (!room.getOwnerId().equals(owner.getId())) {
            throw new GameException(403, "只有房主可以发起再来一局");
        }
        ...
        log.info("再来一局: room={}, newGameId={}, 玩家数={}",
                room.getRoomCode(), game.getId(), room.getPlayerIds().size());
        return getRoomSnapshot(room.getId());
    }
